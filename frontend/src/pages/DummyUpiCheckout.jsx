...
const DummyUpiCheckout = () => {
  ...
  useEffect(() => {
    const fetchUpiDetails = async () => {
      try {
        const response = await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/dummy-upi/generate-upi-qr`);
  ...
  const simulatePayment = async (status) => {
    try {
      const response = await axios.post(`${import.meta.env.VITE_BACKEND_URL}/api/dummy-upi/simulate-payment`, { status });
